"""Post-release pip install test for enforcecore==1.0.23a1."""
import sys
...
def t0():
    import enforcecore
    assert enforcecore.__version__ == "1.0.23a1", f"Got {enforcecore.__version__}"
    assert len(enforcecore.__all__) == 110, f"Got {len(enforcecore.__all__)}"
test("Version and API surface (110 symbols)", t0)

# Test 1: Policy loading
def t1():
    from enforcecore import Policy
    p = Policy.from_dict({"name": "pip-test", "rules": {"allowed_tools": ["read_file", "search"], "denied_tools": ["delete_file"]}})
...
def t3():
    ...
    p = Policy.from_dict({"name": "t3", "rules": {"allowed_tools": ["search"], "denied_tools": ["delete_file"]}})
    @enforce(policy=p)
...
def t12():
    from enforcecore.integrations.langgraph import enforced_tool as lg_tool, wrap_with_policy as lg_wrap
    from enforcecore.integrations.crewai import enforced_tool as cr_tool, wrap_with_policy as cr_wrap
    from enforcecore.integrations.autogen import enforced_tool as ag_tool, wrap_with_policy as ag_wrap
test("Integrations importable", t12)

print()
print("=" * 55)
print(f"RESULTS: {passed} passed, {failed} failed")
print(f"Package: enforcecore==1.0.23a1 from PyPI")
print("=" * 55)
