#!/usr/bin/env python3
"""EnforceCore v1.0.0b2 — Corrected Full Release Audit.
...
TIER1 = [
    "AuditEntry", "Auditor", "ContentViolationError", "CostLimitError",
    "CostTracker", "Decision", "EnforceCoreError", "EnforcementResult",
    "EnforcementViolation", "Enforcer", "KillSwitch", "Policy",
    "PolicyError", "PolicyLoadError", "RateLimiter", "RedactionResult",
    "RedactionStrategy", "Redactor", "ResourceGuard", "ResourceLimitError",
    "SecretScanner", "Settings", "ToolDeniedError", "VerificationResult",
    "__version__", "enforce", "load_policy", "load_trail", "settings",
    "verify_trail",
]
...
TIER2 = [
    "RuleEngine", "JsonlBackend", "NullBackend", "MultiBackend",
    "DomainChecker", "on_pre_call", "on_violation", "ScenarioRunner",
    "BenchmarkRunner", "EnforceCoreInstrumentor", "HookContext",
    "WebhookDispatcher", "AuditRotator", "validate_tool_name",
    "check_input_size", "PolicyEngine", "ContentRule", "DetectedEntity",
    "DetectedSecret", "CustomPattern", "PatternRegistry", "CallContext",
    "RedactionEvent", "ViolationType", "ViolationAction", "Severity",
    "ThreatCategory", "wrap_with_policy", "require_package",
]
...
def t_enforce_allows():
    from enforcecore import enforce, Policy
...
def t_enforce_blocks():
    from enforcecore import enforce, Policy, ToolDeniedError
    ...
    try:
        blocked_fn("test")
        assert False, "Should have raised ToolDeniedError"
...
def t_redactor_email():
    from enforcecore import Redactor, RedactionResult
    r = Redactor()
...
def t_cost_tracker_limit():
    from enforcecore import CostTracker, CostLimitError
    ...
    try:
        ...
        assert False, "Should raise CostLimitError"
...
def t_adapters_importable():
    from enforcecore.integrations import langgraph, crewai, autogen
...
def t_eval_types():
    from enforcecore.eval.types import Scenario, ScenarioResult, ThreatCategory
...
def t_hooks_callable():
    from enforcecore.plugins.hooks import on_pre_call, on_post_call, on_violation, on_redaction
...
def t_exceptions_catchable():
    from enforcecore import (
        EnforceCoreError, PolicyError, PolicyLoadError,
        EnforcementViolation, ToolDeniedError, CostLimitError, ResourceLimitError,
    )
    # Test each, but some need specific init args
    for exc_cls in [PolicyError, PolicyLoadError, EnforcementViolation,
                    ToolDeniedError]:
        try:
            raise exc_cls("test")
...
def t_validate_tool_name():
    from enforcecore.core.hardening import validate_tool_name, InvalidToolNameError
...
def t_check_input_size():
    from enforcecore.core.hardening import check_input_size, InputTooLargeError
...
def t_e2e_pii_redaction():
    ...
    secrets = SecretScanner().detect(result.text)
